...
import com.black.common.utils.StringUtils;
 ...
 * <p/>
 * COMMON PITFALLS:
 * * Make sure you putDouble the exception FIRST in the call.  A common
...
public class Ln {
    ...
    public static int v(Throwable t) {
        return config.minimumLogLevel <= Log.VERBOSE ? print
                .println(Log.VERBOSE, Log.getStackTraceString(t)) : 0;
    }
    ...
    public static int d(Throwable t) {
        return config.minimumLogLevel <= Log.DEBUG ? print
                .println(Log.DEBUG, Log.getStackTraceString(t)) : 0;
    }
    ...
    public static int i(Throwable t) {
        return config.minimumLogLevel <= Log.INFO ? print
                .println(Log.INFO, Log.getStackTraceString(t)) : 0;
    }
    ...
    public static int w(Throwable t) {
        return config.minimumLogLevel <= Log.WARN ? print
                .println(Log.WARN, Log.getStackTraceString(t)) : 0;
    }
    ...
    public static int e(Throwable t) {
        return config.minimumLogLevel <= Log.ERROR ? print
                .println(Log.ERROR, Log.getStackTraceString(t)) : 0;
    }
    ...
    public static class BaseConfig implements Config {
        ...
        public BaseConfig(Context context) {
            try {
                ...
                minimumLogLevel =
                        (flags & ApplicationInfo.FLAG_DEBUGGABLE) != 0 ? Log.VERBOSE : Log.INFO;
        ...
        protected static String getScope(int skipDepth) {
            if (config.minimumLogLevel <= Log.DEBUG) {
                final StackTraceElement trace =
                        Thread.currentThread().getStackTrace()[skipDepth];
                return config.scope + "/" + trace.getFileName() + ":" + trace
                        .getLineNumber();
            }
